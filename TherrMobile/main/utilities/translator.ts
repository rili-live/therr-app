 ...
 * which is how "Friends with Habits" ended up in the Therr build's Google Play prominent
 * disclosure. Those strings take `{appName}` instead.
 *
 ...
 * call site means a new `{appName}` string cannot render as literal "{appName}" just
 * because someone forgot to thread the param through. An explicit `params.appName` still
