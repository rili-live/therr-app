...
- `MainActivity.onCreate` calls `WindowCompat.setDecorFitsSystemWindows(window, false)` before `super.onCreate` so the window is edge-to-edge by the time `react-native-safe-area-context.getConstants()` runs. Without this, the BootTheme phase (which does not inherit `Theme.EdgeToEdge`) leaks zero bottom insets into `initialWindowMetrics` and bottom-anchored UI renders flush against the Android gesture handle.
- Top inset for the custom header comes from `getHeaderTopInset()` in `main/styles/index.ts`, which reads `initialWindowMetrics?.insets?.top` synchronously. The custom header in `Layout.tsx` and the global `ButtonMenu` (`components/ButtonMenu/index.tsx`) additionally subscribe to `SafeAreaInsetsContext` so the actual measured inset replaces the static fallback after the first layout pass — important on Android where `initialWindowMetrics` can resolve to null/zero on cold start.
