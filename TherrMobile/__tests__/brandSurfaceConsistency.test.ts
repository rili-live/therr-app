...
describe('brand surface consistency', () => {
    ...
    it('passes an app name into the Play prominent disclosure', () => {
        // The disclosure has to name the app the user is actually holding — a Play
        // policy requirement, and the Therr build renders these same keys.
        const dictionary = read('main/locales/en-us/dictionary.json');
        const primerModal = read('main/components/Modals/PermissionPrimerModal.tsx');

        expect(dictionary).toContain('{appName}');
        expect(primerModal).toContain('BRAND_DISPLAY_NAME');
    });
