...
- [ ] (2026-07-22, habits-pact-invite-templates) **HABITS pact-invite is broken in production: empty template picker + "couldn't send your pact invite".** Both point at the users-service database being behind the deployed image. Run unconsumed migrations on production users-service (`npm run migrations:run` in `therr-services/users-service`). This applies `20260722000001_habits.habit_goals.reseedTemplates` (self-healing re-seed of the six/seven starter habit templates — the prior `20260510000001` seed skipped silently when the super-admin row was absent and recorded itself complete, so it never re-runs), plus any other pending `habits.*` migrations the invite/nudge write-paths depend on (`20260509000001_habits.pact_members.claimToken`, `20260517000001_habits.pact_members.nudgedAt`, `20260428000001_habits.habit_goals_addGoalType`). After migrating, verify `GET /users-service/habits/goals/templates` returns rows and that a pact invite sends without the error toast. Note: because template loads are a GET, the offline-first axios interceptor swallows any failure into a silent empty list — so an empty picker is expected symptomatology for either a missing seed *or* a failing endpoint.
