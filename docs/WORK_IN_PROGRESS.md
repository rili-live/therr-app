...
- [ ] (2026-07-25, /quality-peer-review) (Optional, no longer required for correctness) One-off backfill to normalize `main.users.phoneNumber` onto a single dialect, plus normalizing on write in `createUser`/`updateUser` so new rows stop diverging. The candidate-matching in `UsersStore` makes the mixed-dialect column work as-is, so this is cleanup rather than a fix — but every future phone lookup has to keep replicating the candidate set until it happens. Note `findUser` (the registration duplicate check) was deliberately left on exact match: widening it would newly block the legitimate personal + creator + business accounts-per-number case.
