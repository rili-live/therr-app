...
- [ ] (2026-08-01, /quality-peer-review) Bump and submit the iOS build for the 3.12.3 release. `TherrMobile/android/app/build.gradle` moved to `versionName 3.12.3` / `versionCode 442`, but `TherrMobile/ios/Therr.xcodeproj/project.pbxproj` is still `MARKETING_VERSION = 1.70.0` / `CURRENT_PROJECT_VERSION = 212`. iOS uses a separate version scheme, so this is a bump-and-submit step, not a value to copy across. (Supersedes the earlier 3.12.1 entry — Android has since moved twice with no matching iOS submission, so the two stores are now two releases apart.)
