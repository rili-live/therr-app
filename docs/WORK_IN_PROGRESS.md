...
- [ ] (2026-08-01, habits-identity-progression) Run `20260801000001_habits.identity_progress` and `20260801000002_habits.identity_reflections` on production users-service (`npm run migrations:run`). Both create new tables in the `habits` schema — nothing existing is altered, so applying them ahead of the image is safe. If the image ships first, every `/habits/identity` route 500s on the missing relation and, more importantly, `createCheckin` logs a warning per check-in (the identity write is wrapped in `recordIdentityVoteSafely`, so check-ins and streaks still succeed). **Run these before or with the users-service deploy.**
- [ ] (2026-08-01, habits-identity-progression) Product follow-up: the top rung of the identity ladder requires a pact-partner affirmation, and there is currently no UI to *give* one. The backend accepts it (`POST /habits/identity/habit/:habitGoalId/reflections` with `reflectionType: 'partner_affirmation'` and `targetUserId`, authorized against a shared active pact) and `IdentityService.getPartnerIdentity` exists to show the partner what to affirm — but until a screen calls them, no user can reach stage 4. Natural home is the pact detail screen, next to the existing nudge action. Until then the ladder tops out at Mindset, which is a coherent state but not the intended one.
- [ ] (2026-08-01, habits-identity-progression) Consider a push notification type for identity stage-ups. Right now a stage-up is a toast (only if the user is in-app when it lands) plus an `identity_stage_up` pact activity row. Reaching Mindset or Identity is a bigger moment than a streak milestone, which already has `PushNotifications.Types.streakMilestone` — adding `identityStageUp` means a new enum value plus message templates in all three `push-notifications-service` locales.
