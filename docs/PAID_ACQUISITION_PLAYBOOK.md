...
|---|---|---|---|
| 2026-09-14 | **API access confirmed; still not spending.** Google Ads API access granted at Basic level (now managed in Cloud Console, not the Ads UI). Both specs pass `campaign apply --validate-only` — the full server-side check of a create, committing nothing. | First live run found the create path had never worked: API v23 renamed the date fields, made an EU political-ads declaration mandatory, and retired standalone Target CPA for Search. Fixed on `general`. A GAQL read of the account found the habits GA4 events **already imported** as 22 conversion actions but all `HIDDEN`; the six `habit_*` events are not imported; property `549794383` still has no Ads link. | The gating items are now all Ads/GA4 UI settings, not credentials or code. Day 0 waits on: un-hide `First open` (+ `phone_verify_success`) for the app arm; mark and import the `habit_*` events; link `549794383` and import `sign_up` for the web arm. The 2026-09-03 verdict — onboarding fix before spend — stands; production is at 1.7.3 / versionCode 44 with the funnel events live. |
