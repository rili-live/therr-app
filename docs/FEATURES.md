...
- **Habit goals** — create from templates or custom; goal-orientation (`goalType`: build_good / break_bad / savings_goal / maintenance)
...
- **Home-screen widget (Android)** — a 4×2 launcher widget (resizable to 4×3) showing the user's weekly rank, XP and daily streak, today's check-ins, and the top 3 of the Friends leaderboard; a user with no connections sees the global board plus an "Invite friends" row. Read-only: it renders a snapshot the app writes after each dashboard refresh or check-in (`main/utilities/habitsWidget.ts` → native `HabitsWidget` module → `widget/HabitsWidgetProvider.kt`) and makes no network calls and holds no token, so it is as fresh as the last app session. It redraws itself every 3h from the snapshot so the reset countdown moves, and after the Monday reset it shows "New week started" instead of last week's rank. Taps deep-link to the Leaderboard (on the board shown), the dashboard's habits tab, or Invite. Cleared on logout. iOS, background refresh and check-in from the widget are not built
