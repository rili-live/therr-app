...
- **Tracked habits & solo habits** — `habits.user_habits` is the registry of habits a user is actually tracking, whether or not a pact backs one. "Solo" is derived (no active pact covers the habit), so a habit whose pact ends quietly becomes personal instead of disappearing. Starting a habit alone unlocks only after the user has **sent** a pact invite — the mandatory-invite onboarding still stands, but a friend who never accepts no longer strands the inviter. Archiving is the lossless way to free a slot; check-ins, streaks and journal entries all survive it
- **Journal** — a day-grouped feed merging five sources newest-first: user-authored notes (`habits.journal_entries`, optionally tagged to a habit), check-ins, earned achievements, streak milestones, and habit starts. Paginated by an exclusive cursor on `occurredAt` rather than an offset, since an offset cannot stay stable across five interleaved sources. Achievements are read through the brand-scoped `UserAchievementsStore`, so a user's Therr achievements never leak into their Habits journal
