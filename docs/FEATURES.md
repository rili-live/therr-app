...
- **City detection on human posts** — a public top-level thought that explicitly names a city (`detectLocality`, matched against the existing `Cities` catalog) is stamped with that city's coordinates, so people's posts join the local feed alongside the bots'. The coordinates are the post's *subject*, never an inference about where the author is: nothing is derived from their home location and nothing is accepted from the request body. Names that double as people or ordinary words (Austin, Phoenix, Charlotte, Mesa) need a place cue — "in Austin", "Austin, TX" — and a post naming two cities is left untagged rather than assigned one of them
- **Location-aware bots** — `therr-ai-automator` accounts with a declared home city write a share of their posts about it, giving a new user's local feed something in it on day one
