...
- **Daily habit reminders** — the digest's reminder pass walks `habits.user_habits` rather than `habits.pacts`, which is what makes it the only notification path that reaches a solo habit or a user whose streak sits at zero (the pact-driven half warns only about a live streak, so a new user and a user who just broke a streak both received nothing at all). One `dailyHabitReminder` per habit that is due today and not yet checked in — a *display* notification, so the OS renders it without the app running; a live streak gets the stronger `streakAtRisk` copy instead. Cadence-aware (`isHabitDueToday` honours `targetDaysOfWeek`, then `frequencyType`/`frequencyCount`, so a 3x/week habit is not nudged seven days a week), taper-aware (an established habit's reminders thin out and then stop, per the lifecycle engine), and deduped per habit per day through `main.notificationQueue`. Kill switch `HABIT_DAILY_REMINDERS_ENABLED=false`; the worker's 5/user/day cap still applies on top
