...
- **Identity progression** — the habit → mindset → identity ladder. Each habit carries a user-authored identity statement ("someone who runs before work") and a monotonic vote count; completed check-ins are votes, and unlike a streak neither the count nor the stage ever resets. Five rungs (Intention → Repetition → Automaticity → Mindset → Identity), each gated on a *different kind* of evidence so volume alone can't reach the top: consistency ratio + week spread, falling `difficultyRating`, a recovered lapse, elapsed calendar time, a 1–5 self-concept self-report, and a pact-partner affirmation. Reflection prompts (why / recommitment / obstacle / self-concept) are selected server-side under per-type cooldowns, at most one per check-in. Rules live in `therr-js-utilities/config/habits`; state in `habits.identity_progress` + `habits.identity_reflections`; API at `/habits/identity`
