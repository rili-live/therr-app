/**
 * Centralized runtime configuration for the maps service.
 *
 * See users-service/src/config.ts for the pattern. Validation runs at boot and
 * fails fast if a required var is missing.
 */

type RequiredKey =
    | 'MAPS_SERVICE_API_PORT'
    | 'MAPS_SERVICE_DATABASE'
    | 'DB_HOST_MAIN_READ'
    | 'DB_HOST_MAIN_WRITE'
    | 'DB_USER_MAIN_READ'
    | 'DB_USER_MAIN_WRITE'
...
export const validateEnv = (): void => {
    ...
    if (missing.length) {
        throw new Error(
            `maps-service: missing required environment variables: ${missing.join(', ')}`,
        );
    }
};
...
const config = {
    ...
    port: getStr('MAPS_SERVICE_API_PORT'),
    uriWhitelist: getStr('URI_WHITELIST').split(',').filter(Boolean),
    db: {
        name: getStr('MAPS_SERVICE_DATABASE'),
        read: {
