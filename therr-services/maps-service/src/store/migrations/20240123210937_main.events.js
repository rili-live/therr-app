...
exports.up = (knex) => installExtensions(knex)
    ...
    .then(() => knex.schema.withSchema('main').createTable('events', async (table) => {
        ...
        table.string('areaType', 25).notNullable().defaultsTo('events');
        table.bool('isDraft').notNullable().defaultTo(false);
        table.timestamp('scheduleStartAt', { useTz: true }).notNullable();
        table.timestamp('scheduleStopAt', { useTz: true }).notNullable();
