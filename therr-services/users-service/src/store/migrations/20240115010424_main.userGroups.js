exports.up = async (knex) => knex.schema.dropTable('main.userForums')
    .then(() => knex.schema.withSchema('main').createTable('userGroups', (table) => {
...
exports.down = (knex) => knex.schema.dropTable('main.userGroups')
    .then(() => knex.schema.withSchema('main').createTable('userForums', (table) => {
        table.uuid('id').primary().notNullable().defaultTo(knex.raw('uuid_generate_v4()'));
        table.uuid('userId')
            .references('id')
            .inTable('main.users')
