exports.up = (knex) => knex.schema.raw('CREATE EXTENSION IF NOT EXISTS "uuid-ossp";')
    .then(() => knex.schema.withSchema('main').createTable('thoughtReactions', (table) => {
        table.uuid('id').notNullable().defaultTo(knex.raw('uuid_generate_v4()'));
        table.uuid('thoughtId').notNullable();
        ...
        // Indexes
        table.unique(['thoughtId', 'userId']);
        table.index(['thoughtId', 'userId']);
    }));

exports.down = (knex) => knex.schema.withSchema('main').dropTable('thoughtReactions');
