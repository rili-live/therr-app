...
import MomentReactionsStore from '../../src/store/MomentReactionsStore';

describe('MomentReactionsStore', () => {
    ...
    describe('getCounts', () => {
        it('returns empty array when no moment IDs provided', async () => {
            const mockStore = createMockStore();
            const store = new MomentReactionsStore(mockStore);
        ...
        it('generates query with correct table and groupBy', async () => {
            ...
            store.getCounts(['moment-1', 'moment-2'], {});

            const query = mockStore.read.query.args[0][0];
            expect(query).to.include('from "main"."momentReactions"');
            expect(query).to.include('group by "momentId"');
            expect(query).to.include('"userHasLiked" = true');
            expect(query).to.include("'moment-1'");
            expect(query).to.include("'moment-2'");
        });
        ...
        it('uses custom countBy parameter', async () => {
            ...
            store.getCounts(['moment-1'], {}, 'userHasSuperLiked');
        ...
        it('includes additional conditions in query', async () => {
            ...
            store.getCounts(['moment-1'], { someCondition: true });
    ...
    describe('get', () => {
        ...
        it('includes momentIds filter when provided', async () => {
            const mockStore = createMockStore();
            const store = new MomentReactionsStore(mockStore);

            store.get({ userId: 'user-1' }, ['m-1', 'm-2']);

            const query = mockStore.read.query.args[0][0];
            expect(query).to.include('"momentId" in');
            expect(query).to.include("'m-1'");
            expect(query).to.include("'m-2'");
        });
    ...
    describe('getByMomentId', () => {
        it('generates query with conditions and default limit', async () => {
            const mockStore = createMockStore();
            const store = new MomentReactionsStore(mockStore);

            store.getByMomentId({ momentId: 'moment-123' });

            const query = mockStore.read.query.args[0][0];
            expect(query).to.include('select *');
            expect(query).to.include('from "main"."momentReactions"');
            expect(query).to.include('"momentId" = \'moment-123\'');
            expect(query).to.include('limit 100');
        });
        ...
        it('restricts limit to 1000', async () => {
            ...
            store.getByMomentId({ momentId: 'moment-123' }, 5000);
        ...
        it('uses provided limit when under 1000', async () => {
            ...
            store.getByMomentId({ momentId: 'moment-123' }, 50);
    ...
    describe('create', () => {
        it('generates insert query for single reaction', async () => {
            ...
            expect(query).to.include('insert into "main"."momentReactions"');
            expect(query).to.include("'moment-1'");
        ...
        it('generates insert query for batch reactions', async () => {
            ...
            store.create([
                { momentId: 'moment-1', userId: 'user-1' },
                { momentId: 'moment-2', userId: 'user-1' },
            ]);
    ...
    describe('update', () => {
        it('generates update query with conditions', async () => {
            ...
            const query = mockStore.write.query.args[0][0];
            expect(query).to.include('update "main"."momentReactions"');
            expect(query).to.include('"userHasLiked" = true');
            expect(query).to.include('"momentId" = \'moment-1\'');
            expect(query).to.include('"userId" = \'user-1\'');
        ...
        it('includes whereIn clause when provided', async () => {
            ...
            store.update(
                ...
                { userHasLiked: true },
                {
                    ...
                    whereInArray: [['user-1', 'moment-1'], ['user-2', 'moment-2']],
                },
            );

            const query = mockStore.write.query.args[0][0];
            expect(query).to.include('("userId", "momentId") in');
            expect(query).to.include("'user-1'");
            expect(query).to.include("'moment-1'");
        });
    });
    ...
    describe('delete', () => {
        it('generates delete query', async () => {
            ...
            const query = mockStore.write.query.args[0][0];
            expect(query).to.include('delete from "main"."momentReactions"');
