/**
 * Centralized runtime configuration for the messages service.
 * See users-service/src/config.ts for the pattern.
 */

type RequiredKey =
    | 'MESSAGES_SERVICE_API_PORT'
    | 'MESSAGES_SERVICE_DATABASE'
    | 'DB_HOST_MAIN_READ'
    | 'DB_HOST_MAIN_WRITE'
    | 'DB_USER_MAIN_READ'
    | 'DB_USER_MAIN_WRITE'
...
export const validateEnv = (): void => {
    ...
    if (missing.length) {
        throw new Error(
            `messages-service: missing required environment variables: ${missing.join(', ')}`,
        );
    }
};
...
export const config = {
    ...
    port: getStr('MESSAGES_SERVICE_API_PORT'),
    uriWhitelist: getStr('URI_WHITELIST').split(',').filter(Boolean),
    db: {
        name: getStr('MESSAGES_SERVICE_DATABASE'),
        read: {
